...
foreach my $key (sort keys(%{$hash})) {
	...
	$content .= "chr$chr\t$pos\t.\t$ref\t$alt\t.\t.\tHET=$hash->{$key}->{'heterozygous'};HOM=$hash->{$key}->{'homozygous'};HEM=$hash->{$key}->{'hemizygous'};LED_URL=https://ushvam.iurc.montp.inserm.fr/perl/led/variant.pl?var=$id\n";
}
