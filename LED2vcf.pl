...
foreach my $key (sort keys(%{$hash})) {
	...
	$content .= "chr$chr\t$pos\t.\t$ref\t$alt\t.\t.\tHET=$hash->{$key}->{'heterozygous'};HOM=$hash->{$key}->{'homozygous'};HEM=$hash->{$key}->{'hemizygous'};MAX=".max($hash->{$key}->{'heterozygous'}, $hash->{$key}->{'homozygous'}, $hash->{$key}->{'hemizygous'}).";LED_URL=https://ushvamdev.iurc.montp.inserm.fr/perl/led/variant.pl?var=$id\n";
}
